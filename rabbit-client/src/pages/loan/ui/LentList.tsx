import { mockLoanData } from "@/entities/loan/mocks/data";
import LoanInfo from "@/entities/loan/ui/LoanInfo";
import LoanInfoMobile from "@/entities/loan/ui/LoanInfoMobile";
...
const LentList = () => {
  ...
  if (!mockLoanData || mockLoanData.length === 0) {
    return (
  ...
  return (
    <section className="flex flex-col gap-8">
      ...
      <h2 className="text-xl font-semibold">대출 내역</h2>
      <div className="w-full overflow-hidden rounded-lg bg-gray-900">
        {isDesktop ? (
          <LoanInfo data={mockLoanData} />
        ) : (
          <LoanInfoMobile data={mockLoanData} />
        )}
      </div>
    </section>
  );
