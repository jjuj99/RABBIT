...
/**
 * 다크 테마 차용증 계약서 PDF 생성 서비스
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class DarkThemePdfService {
    ...
    // 색상 정의 (SVG에서 사용된 색상)
    private static final Color BACKGROUND_COLOR = new DeviceRgb(26, 26, 26); // #1a1a1a
    private static final Color HEADER_COLOR = new DeviceRgb(34, 34, 34); // #222
    private static final Color PRIMARY_COLOR = new DeviceRgb(0, 255, 102); // #0f6
    private static final Color TEXT_COLOR = ColorConstants.WHITE;
    private static final Color MUTED_TEXT_COLOR = new DeviceRgb(192, 192, 192); // silver
    private static final Color BORDER_COLOR = new DeviceRgb(71, 71, 71); // #474747
    private static final Color DESTRUCTIVE_COLOR = new DeviceRgb(255, 70, 70); // #ff4646
    private static final Color TABLE_EVEN_ROW = new DeviceRgb(40, 40, 40); // #282828
    private static final Color TABLE_ODD_ROW = new DeviceRgb(51, 51, 51); // #333
    ...
    /**
     * 계약 상세 정보를 바탕으로 다크 테마 차용증 PDF 생성
     * @param contract 계약 엔티티
     * @return PDF 파일 바이트 배열
     */
    public byte[] generateDarkThemeContractPdf(Contract contract) {
        log.debug("다크 테마 차용증 PDF 생성: 계약 ID = {}", contract.getContractId());
        ...
        try {
            ...
            // 기존 이벤트 핸들러를 사용하여 모든 페이지에 배경색 적용
    ...
    /**
     * 암호화된 다크 테마 차용증 PDF 생성 (IPFS 업로드용)
     * @param contract 계약 엔티티
     * @return 암호화된 PDF 파일 바이트 배열
     */
    public byte[] generateEncryptedDarkThemeContractPdf(Contract contract) {
        log.debug("암호화된 다크 테마 차용증 PDF 생성: 계약 ID = {}", contract.getContractId());
     ...
     * 페이지 배경색 설정 (새 메서드 추가)
     */
    private void setPageBackground(PdfPage page, Color backgroundColor) {
        Rectangle rect = new Rectangle(0, 0, page.getPageSize().getWidth(), page.getPageSize().getHeight());
    ...
    /**
     * 값 셀의 줄바꿈을 강제하기 위한 또 다른 대안적 방법
     * - 이 방법이 앞선 방법으로도 해결되지 않을 경우 사용
     */
    private Cell createValueCellWithWordWrap(String value, Color backgroundColor, Color textColor) {
        Cell valueCell = new Cell();
        // 셀 자체에 줄바꿈 속성 지정
        valueCell.setProperty(com.itextpdf.layout.properties.Property.OVERFLOW_X, com.itextpdf.layout.properties.OverflowPropertyValue.VISIBLE);
        valueCell.setProperty(com.itextpdf.layout.properties.Property.OVERFLOW_Y, com.itextpdf.layout.properties.OverflowPropertyValue.VISIBLE);
        ...
        // - 한글은 약 20-25자, 영문은 약 30-40자 정도가 적당
        ...
        // 추가적인 줄바꿈 속성 지정
        valueParagraph.setProperty(com.itextpdf.layout.properties.Property.OVERFLOW_X, com.itextpdf.layout.properties.OverflowPropertyValue.FIT);
        valueParagraph.setWidth(UnitValue.createPercentValue(100));

        // 셀에 단락 추가
        valueCell.add(valueParagraph);
        if(backgroundColor!=null) valueCell.setBackgroundColor(backgroundColor);
    ...
    /**
     * 이 방법을 사용할 때의 addPersonInfoSection 메서드 수정 예시
     */
    ...
    private void addHeader(Document document, String title, PdfFont boldFont) throws IOException {
        ...
        try {
            ...
            // 로고 컨테이너 (녹색 배경에 R 텍스트)
            Cell logoCell = new Cell();

            // 녹색 배경의 R 로고
            Div logoContainer = new Div();
            logoContainer.setBackgroundColor(PRIMARY_COLOR);
    ...
    private void addLoanDetails(Document document, Contract contract, PdfFont boldFont) {
        ...
        addTableRow(table, "연체 이자율", contract.getDefaultInterestRate() + "%", boldFont, 6, DESTRUCTIVE_COLOR);
    ...
    private void addTableRow(Table table, String key, String value, PdfFont boldFont, int rowIndex, Color valueColor) {
        ...
        valueCell.add(valueParagraph);
        valueCell.setBackgroundColor(rowIndex % 2 == 0 ? TABLE_ODD_ROW : TABLE_EVEN_ROW);
        valueCell.setBorder(new SolidBorder(BORDER_COLOR, 1));
    ...
    private void addSignatureSection(Document document, Contract contract, PdfFont boldFont) {
        ...
        Cell creditorNameCell = createValueCellWithWordWrap(creditorName, null, TEXT_COLOR);
        ...
        Cell walletCell = createValueCellWithWordWrap(creditorWallet, null, TEXT_COLOR);
        ...
        Cell debtorNameCell = createValueCellWithWordWrap(debtorName, null, TEXT_COLOR);
        ...
        Cell debtorWalletCell = createValueCellWithWordWrap(debtorWallet, null, TEXT_COLOR);
        ...
        if (contract.getTokenId() != null) {
            ...
            // 토큰 ID에도 줄바꿈 적용
            Cell tokenIdCell = createValueCellWithWordWrap("토큰 ID: " + contract.getTokenId(), null, TEXT_COLOR);
    ...
    private void addEncryptedSignatureSection(Document document, Contract contract, PdfFont boldFont) {
        ...
        // 암호화된 채권자 패스키 (값) - 줄바꿈 적용
        String encryptedCreditorName = encryptionService.encryptWithPrefix(contract, contract.getCreditor().getPassCode());
        Cell creditorNameCell = createValueCellWithWordWrap(encryptedCreditorName, null, TEXT_COLOR);
        creditorNameCell.setBorder(Border.NO_BORDER);
        creditorCell.add(creditorNameCell);
        ...
        // 암호화된 채무자 패스키 (값) - 줄바꿈 적용
        String encryptedDebtorName = encryptionService.encryptWithPrefix(contract, contract.getDebtor().getPassCode());
        Cell debtorNameCell = createValueCellWithWordWrap(encryptedDebtorName, null, TEXT_COLOR);
        debtorNameCell.setBorder(Border.NO_BORDER);
        debtorCell.add(debtorNameCell);
