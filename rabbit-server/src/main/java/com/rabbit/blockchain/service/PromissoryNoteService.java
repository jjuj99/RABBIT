...
public class PromissoryNoteService {
    ...
    public PromissoryNote.PromissoryMetadata getPromissoryMetadata(BigInteger tokenId) throws Exception {
        ...
        // 2. 함수 호출 및 결과 반환
        PromissoryNote.PromissoryMetadata metadata = contract.getPromissoryMetadata(tokenId).send();

        // 3. 로깅 (선택)
        log.info("조회된 PromissoryMetadata: tokenId={}, creditor={}, debtor={}",
                tokenId,
                metadata.crInfo.crName,
                metadata.drInfo.drName);
